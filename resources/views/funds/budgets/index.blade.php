        ...
        @if(isset($monthlyIncomeTotal) || isset($monthlyExpenseTotal))
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <p class="text-sm font-medium text-slate-500 dark:text-slate-400">月总支出</p>
                    <p class="mt-2 text-2xl font-bold text-red-600 dark:text-red-400">-¥{{ number_format($monthlyExpenseTotal, 2) }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <p class="text-sm font-medium text-slate-500 dark:text-slate-400">月总收入</p>
                    <p class="mt-2 text-2xl font-bold text-emerald-600 dark:text-emerald-400">¥{{ number_format($monthlyIncomeTotal, 2) }}</p>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <p class="text-sm font-medium text-slate-500 dark:text-slate-400">月净额</p>
                    <p class="mt-2 text-2xl font-bold {{ ($monthlyIncomeTotal - $monthlyExpenseTotal) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">¥{{ number_format($monthlyIncomeTotal - $monthlyExpenseTotal, 2) }}</p>
                </div>
            </div>
        @endif
                        ...
                        @forelse ($budgets as $budget)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                                ...
                                <td data-label="月金额" class="px-4 py-4 align-middle">
                                    @php $monthlyDisplay = $budget->type === 'expense' ? -$budget->monthly_amount : $budget->monthly_amount; @endphp
                                    <span class="text-sm font-semibold {{ $budget->type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
                                        {{ $budget->type === 'expense' ? '-' : '' }}¥{{ number_format(abs($monthlyDisplay), 2) }}
                                    </span>
                                </td>
                                <td data-label="年金额" class="px-4 py-4 align-middle">
                                    @php $annualDisplay = $budget->type === 'expense' ? -$budget->annual_amount : $budget->annual_amount; @endphp
                                    <span class="text-sm {{ $budget->type === 'income' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
                                        {{ $budget->type === 'expense' ? '-' : '' }}¥{{ number_format(abs($annualDisplay), 2) }}
                                    </span>
                                </td>
